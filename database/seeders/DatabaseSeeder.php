...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $raw1 = RawMaterial::firstOrCreate(
            ['code' => 'RM-01'],
            [
        ...
        $fin1 = FinishedProduct::firstOrCreate(
            ['code' => 'FG-01'],
            [
        ...
        // 6. Sample Purchase (Hanya dibuat jika belum ada)
        // Beli 10 ton garam mentah = 10.000 kg = 10.000.000 gram
        $purchaseInvoice = 'PB-' . date('Ymd') . '-001';
        if (Purchase::where('invoice_number', $purchaseInvoice)->doesntExist()) {
        ...
        // 7. Sample Production: Produksi 100 bungkus (Hanya dibuat jika belum ada)
        $prodNumber = 'PR-' . date('Ymd') . '-001';
        if (Production::where('production_number', $prodNumber)->doesntExist()) {
            $prodPacks = 100;
        ...
        // 8. Sample Sales (Hanya dibuat jika belum ada)
        $txNumber = 'PJ-' . date('Ymd') . '-001';
        if (Sale::where('transaction_number', $txNumber)->doesntExist()) {
            $salePacks = 40;
            $unitPrice = 3500;
